/**
 * ESP-NOW use case example
 * Modified for continuous packets broadcasting
 */
#include <stdlib.h>
#include <time.h>
#include <string.h>
...
void espnow_data_prepare(example_espnow_send_param_t *send_param)
{
    ...
    //buf->type = IS_BROADCAST_ADDR(send_param->dest_mac) ? ESPNOW_DATA_BROADCAST : EXAMPLE_ESPNOW_DATA_UNICAST;
    ...
    int remaining_bytes = send_param->len - sizeof(espnow_data_t);
    esp_fill_random(buf->payload, remaining_bytes);

    buf->crc = crc16_le(UINT16_MAX, (uint8_t const *) buf, (uint32_t) send_param->len);
}
...
static void espnow_task(void *pvParameter)
{
    ...
    if (esp_now_send(send_param->dest_mac, send_param->buffer, (size_t) send_param->len) != ESP_OK)
    {
                ...
                if (ret == ESPNOW_DATA_BROADCAST)
                {
                    ...
                    if (recv_state == 1)
                    {
                        ...
                        if (send_param->unicast == false && send_param->magic >= recv_magic)
                        {
                            ...
                            if (esp_now_send(send_param->dest_mac, send_param->buffer, (size_t) send_param->len) !=
                                ESP_OK)
                            {
                                ESP_LOGE(TAG, "Send error");
                                example_espnow_deinit(send_param);
